...
var getList = function(params,tab_index,lmt){
    ...
    if(tab_index=='0'){
        params+='&newsCategory=2&newsCategory2=31';
    } else if(tab_index == '1') {
        params+='&newsCategory=2&contentType=26';
        url = SITEURL + "dashboard/kroo_news_video";
    } 
...
$(document).ready(function () {
    ...
    getList('',1,0); //Kroo Video
    //getList('',2,0); //Kroo Video
    //getList('',3,0);
    //getList('',2,0);
    //getList('',4,0);
    //getListWaiver('',3,0);
...
$(document).on('click','.btn_edit_news',function(){
    ...
    $('input[name="news_img"]').val('');
    var id = $(this).data("id");
    var news_category = $(this).data("news_category");
    ...
    var news_tags = $('#news_tags_'+id).val();
    ...
    $('#edit_news_category').val(news_category);
    $('#news_image_url').val(img_url);
    $('#edit_news_source').val(news_source);
    $('#edit_news_tags').val(news_tags);
...
$(document).on('click','#btn_update',function(e){ 
    ...
    var edit_news_category = $('#edit_news_category').val();
    

        if(((news_leagues_id == 0) || (news_leagues_id == null))  && edit_news_category==2 ) {
            alert("Please Select league");   
    
        }else if(((news_team_id == 0) || (news_team_id == null))  && edit_news_category==2 ) { 
            ...
            var news_tags  = $('#edit_news_tags').val();
...
$(document).on('click','.btn_copy_news',function(){
    ...
    var tab_index = $('.nav-tabs > li.active').index();
    $('#copy_team_id').html('');
    $('#copy_news_category').val(news_category);
    if(news_category == 31){
                
        $('#copy_team_list').css({'display':''});
        $('#myCopyModal').find('#modal_title').html("Copy and publish to Kroo"); 
        ...
        $('#myCopyModal').find('#modal_title').html("Copy and publish to headlines");                
    }
...
$(document).on('click','#btn_copy_update',function(e){
    ...
    var news_category = $('#copy_news_category').val();
    var tab_index = $('.nav-tabs > li.active').index();
    if(team_id == 0 && news_category == 31){
        alert("Please Select Team");
    }else{
        var params = '&news_id='+news_id+'&team_id='+team_id+'&news_category='+news_category;
...
$(document).on('click','.btn_publish_news',function(){
    ...
    if(news_category==2){
        var team_check_url = SITEURL + "dashboard/get_rss_news_detail/"+news_id;
        timer = 2000;
        $.ajax({
            url:team_check_url
        }).done(function(getdata){
    ...
    setTimeout(function(){
        if(team_id!=null && team_id == 0 && tab_index==1){ 
...
$(document).on('click','.publis',function(evt){  
    evt.stopPropagation();
    myFunction($(this).parents('.dropbtn'));
});

/* When the user clicks on the button, 
toggle between hiding and showing the dropdown content */
function myFunction(th) {
    $(".myDropdown").toggleClass("show",false);
    $(th).next('#myDropdown').toggleClass("show",true);
}
                   ...
                   // var ele   = $('#gameAdd');
                    ...
                    $.ajax({
                      ...
                      processData: false
                        ...
                        $('#btn_game_add_'+news_id).find('span').css({'color':'#00AC14;'});
                        // location.reload();
                        if(publish_id==0){         
