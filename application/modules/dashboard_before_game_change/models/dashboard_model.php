...
class Dashboard_model extends Model {
    ...
    public function kroo_news_list($channelID,$leagueID,$teamID,$newsType,$dateFrom,$dateTo,$news_category=2){
        ...
        if($leagueID>0 && $news_category==2){
            $this->db->where('l.id',$leagueID);
            $maxId+=500;
        }elseif($leagueID>0){
        ...
        $this->db->where("(rsn.is_deleted !='1' AND rsn.news_category = $news_category)");
    ...
    public function copy_news($news_id,$team_id,$tab_index){
    ...
    public function getFantasySummary() {
        $data = $this->db->query('SELECT  (SELECT COUNT(id) FROM   rss_player_news) AS total_f_feeds, (SELECT COUNT(published_at) FROM   player_news WHERE published_at != 0) AS total_f_published');
        if ($data->num_rows() > 0) {
            return $data->result();
        } else {
            return null;
    ...
    public function update_news($news_id,$news_title,$news_summary,$news_url,$edited_by_id,$team_id,$news_source,$tab_index){
        ...
        if($query->num_rows() > 0){
                $query = $this->db->query("UPDATE sports_news SET title='$news_title',summary='$news_summary',url= '$news_url',source='$news_source', edited_by_admin_id=$edited_by_id,content_type= '$content_type' WHERE rss_sports_news_id=$news_id");
                return $query;
        }else{
           ...
           // $content_type = $row->content_type;         
            $query = $this->db->query("INSERT INTO sports_news (rss_sports_news_id,title,summary,url,source,edited_by_admin_id,channel_published_at, channel,news_category,content_type,image) VALUES ('$news_id', '$news_title', '$news_summary', '$news_url','$news_source','$edited_by_id',UNIX_TIMESTAMP(),'$channel','$news_category','$content_type','$image')");
    ...
    public function update_player_news($news_id,$news_title,$news_summary,$news_url,$edited_by_id){
       $query = $this->db->query("SELECT * FROM player_news WHERE rss_player_news_id='$news_id'");
        if($query->num_rows() > 0){
            $query = $this->db->query("UPDATE player_news SET title='$news_title',summary='$news_summary', url= '$news_url', edited_by_admin_id=$edited_by_id 
                WHERE rss_player_news_id='$news_id'");
            return $query;
        }else{
            $news_query = $this->db->query("SELECT * FROM rss_player_news WHERE id='$news_id'");
            $row = $news_query->row();
            $source = $row->source;
            $channel = $row->channel;
            $name_abbr = $row->name_abbr;
            $player = $this->db->query("SELECT id as player_id, league_id, team_id FROM players WHERE name_abbr = '$name_abbr'");
            $row1 = $player->row();
            $player_id = $row1->player_id;
            $league_id = $row1->league_id;
            $team_id = $row1->team_id;
            $query = $this->db->query("INSERT INTO player_news (rss_player_news_id,title,summary,url,edited_by_admin_id,source,channel) 
                VALUES ($news_id, '$news_title', '$news_summary', '$news_url', $edited_by_id,'$source',$channel)");
            $insert_id = $this->db->insert_id(); 
        ...
        // else if($tab_index == 2){
        //     $query = $this->db->query("INSERT INTO rss_player_news (league_id,title,summary,url,channel_published_at, channel,news_category) 
        //         VALUES ($team_id,'$news_title', '$news_summary', '$news_url',UNIX_TIMESTAMP(),59,2)");                      
        //         return $query;
        // }
    }

    public function add_player_news($tab_index,$news_title,$news_summary,$news_url,$league_id,$name_abbr,$team_code,$img){
        if($tab_index == 2){
                $query = $this->db->query("INSERT INTO rss_player_news (league_id, title, summary, url, image, channel_published_at, channel,team_code,name_abbr) 
                VALUES ($league_id,'$news_title', '$news_summary', '$news_url', '$img', UNIX_TIMESTAMP(),64,'$team_code','$name_abbr')");                      
                return $query;
            }else{
                 $query = $this->db->query("INSERT INTO rss_waiver_news (league_id, title, summary, url, image, channel_published_at, channel,name_abbr) 
                VALUES ($league_id,'$news_title', '$news_summary', '$news_url', '$img', UNIX_TIMESTAMP(),64,'$name_abbr')");                      
                return $query;
            }   
    }

    public function get_player_suggestion($keyword){
         $query = $this->db->query("SELECT p.name_full,t.code FROM players as p LEFT JOIN teams as t ON p.team_id = t.id WHERE name_full like '" . $keyword . "%' ORDER BY name_full LIMIT 0,6");                      
                return $query->result();
    }

    public function update_waiver_news($news_id,$news_title,$news_summary,$news_url,$edited_by_id){
       $query = $this->db->query("SELECT * FROM waiver_news WHERE rss_waiver_news_id='$news_id'");
        if($query->num_rows() > 0){
            $query = $this->db->query("UPDATE waiver_news SET title='$news_title',summary='$news_summary', url= '$news_url', edited_by_admin_id=$edited_by_id 
                WHERE rss_waiver_news_id='$news_id'");
            return $query;
        }else{
            $news_query = $this->db->query("SELECT * FROM rss_waiver_news WHERE id='$news_id'");
            $row = $news_query->row();
            $source = $row->source;
            $image  = $row->image;
            //$news_category = $row->news_category;   
            $league_id = $row->league_id; 
            $query = $this->db->query("INSERT INTO waiver_news (rss_waiver_news_id,title,summary,image,url,edited_by_admin_id,source) 
                VALUES ($news_id, '$news_title', '$news_summary','$image', '$news_url', $edited_by_id,'$source')");
            $insert_id = $this->db->insert_id();      
            
            $this->db->query("INSERT INTO waiver_news_leagues (news_id,league_id) VALUES ($insert_id,$league_id)");    
    ...
    public function newsPublish($newsIdFor,$user_id,$tab_index){
        if($tab_index == 0 || $tab_index == 1 || $tab_index == 4){
            ...
            $query = $this->db->query("SELECT * FROM player_news WHERE rss_player_news_id='$newsIdFor'");
            if($query->num_rows() > 0){
            $this->db->query("UPDATE player_news SET published_at = UNIX_TIMESTAMP() WHERE rss_player_news_id='$newsIdFor'");
            return 'update';
            }else{
            $result = $this->db->query("SELECT * FROM rss_player_news WHERE id='$newsIdFor'");
            return $result->result_array();
            }  
        }else if($tab_index == 3){
            $query = $this->db->query("SELECT * FROM waiver_news WHERE rss_waiver_news_id='$newsIdFor'");
            if($query->num_rows() > 0){
            $this->db->query("UPDATE waiver_news SET published_at = UNIX_TIMESTAMP() WHERE rss_waiver_news_id='$newsIdFor'");
            return 'update';
            }else{
                $result = $this->db->query("SELECT * FROM rss_waiver_news WHERE id='$newsIdFor'");
                return $result->result_array();
            }  
        }
    ...
    public function newsPublishNotified($news_id, $user_id, $tab_index){
        if($tab_index == 0 || $tab_index == 1 || $tab_index == 4){
            $query = $this->db->query("SELECT * FROM sports_news WHERE rss_sports_news_id = $news_id");
            if($query->num_rows() > 0){
            ...
            $query = $this->db->query("SELECT * FROM player_news WHERE rss_player_news_id = $news_id");
            if($query->num_rows() > 0){
                $this->db->query("UPDATE player_news SET notified_at = UNIX_TIMESTAMP(), published_at = UNIX_TIMESTAMP() WHERE rss_player_news_id = $news_id");
                return 'update';
            }else{
                $result = $this->db->query("SELECT * FROM rss_player_news WHERE id= $news_id ");
                return $result->result_array();
            }  
        }elseif($tab_index == 3){
            $query = $this->db->query("SELECT * FROM waiver_news WHERE rss_waiver_news_id = $news_id");
            if($query->num_rows() > 0){
                $this->db->query("UPDATE waiver_news SET notified_at = UNIX_TIMESTAMP(), published_at = UNIX_TIMESTAMP() WHERE rss_waiver_news_id = $news_id");
                return 'update';
            }else{
                $result = $this->db->query("SELECT * FROM rss_waiver_news WHERE id= $news_id ");
                return $result->result_array();
            }  
        }
    }

    public function insert_published_news($news_data,$news_id,$team_id,$tab_index,$league_id){
        if($tab_index == 0 || $tab_index == 4){
    ...
    public function newspublishedfantasy($fnews_id, $user_id){
        $query = $this->db->query("SELECT * FROM player_news WHERE rss_player_news_id='$fnews_id'");
        if($query->num_rows() > 0){
            $this->db->query("UPDATE player_news SET published_at = UNIX_TIMESTAMP() WHERE rss_player_news_id='$fnews_id'");
            return 'update';
        }else{
            $result = $this->db->query("SELECT * FROM rss_player_news WHERE id='$fnews_id'");
            return $result->result_array();
        }  
    }
    ...
    public function newspublishedarticle($fnews_id, $user_id){
        $query = $this->db->query("SELECT * FROM waiver_news WHERE rss_waiver_news_id='$fnews_id'");
        if($query->num_rows() > 0){
            $this->db->query("UPDATE waiver_news SET published_at = UNIX_TIMESTAMP() WHERE rss_waiver_news_id='$fnews_id'");
            return 'update';
        }else{
            $result = $this->db->query("SELECT * FROM rss_waiver_news WHERE id='$fnews_id'");
            return $result->result_array();
        }   
    }
    ...
    public function update_notify($news_id,$tab_index){
        if($tab_index==0 || $tab_index==1 || $tab_index==4){
            ...
            $query = $this->db->query("SELECT * FROM player_news WHERE rss_player_news_id ='$news_id'");
            if($query->num_rows() > 0){
                $this->db->query("UPDATE player_news SET notified_at = UNIX_TIMESTAMP() WHERE rss_player_news_id = $news_id ");
                return 'notified';
            }
        }elseif($tab_index==3){
            $query = $this->db->query("SELECT * FROM waiver_news WHERE rss_waiver_news_id ='$news_id'");
            if($query->num_rows() > 0){
                $this->db->query("UPDATE waiver_news SET notified_at = UNIX_TIMESTAMP() WHERE rss_waiver_news_id = $news_id ");
                return 'notified';
            }
        }
    }
    ...
    function getDeviceInfo($newsIdForNotify, $tab_index,$level ) {    
        $notilevel="";
        if($level==1){
    ...
    public function hide_kroo_news($news_id, $tab_index){  
        if($tab_index == 0 || $tab_index == 1 || $tab_index == 4){
            ...
            $this->db->query("UPDATE rss_player_news SET is_deleted=1 WHERE id = $news_id");
            $this->db->query("UPDATE player_news SET is_deleted=1 WHERE rss_player_news_id = $news_id ");
            return 'fantasy';
        }else{
            $this->db->query("UPDATE rss_waiver_news SET is_deleted=1 WHERE id = $news_id");
            $this->db->query("UPDATE waiver_news SET is_deleted=1 WHERE rss_waiver_news_id = $news_id ");   
            return 'fantasy';
        }
    ...
    public function news_publish_activity($news_id, $tab_index, $user_id){
        ...
        if($tab_index == 0 || $tab_index == 4){
            ...
            $query = $this->db->query("SELECT * FROM admin_activity WHERE activity_news_id = $news_id AND activity_user_id=$user_id AND activity_news_type='fantasy' AND activity_action='publish'");
            if($query->num_rows() == 0){
                return $this->db->query("INSERT INTO admin_activity (activity_news_id, activity_user_id, activity_news_type, activity_action) 
                                        VALUES ($news_id, $user_id, 'fantasy', 'publish') ");
            }
            
        }
        else{

            $query = $this->db->query("SELECT * FROM admin_activity WHERE activity_news_id = $news_id AND activity_user_id=$user_id AND activity_news_type='waiver' AND activity_action='publish'");
            if($query->num_rows() == 0){
                return $this->db->query("INSERT INTO admin_activity (activity_news_id, activity_user_id, activity_news_type, activity_action) 
                                        VALUES ($news_id, $user_id, 'waiver', 'publish') ");
            }
    ...
    public function news_hide_activity($news_id, $tab_index, $user_id){
        ...
        if($tab_index == 0){
            ...
            $query = $this->db->query("SELECT * FROM admin_activity WHERE activity_news_id = $news_id AND activity_user_id=$user_id AND activity_news_type='fantasy' AND activity_action='delete'");
            if($query->num_rows() == 0){
                return $this->db->query("INSERT INTO admin_activity (activity_news_id, activity_user_id, activity_news_type, activity_action) 
                                        VALUES ($news_id, $user_id, 'fantasy', 'delete') ");
            }
            
        }
        else{

            $query = $this->db->query("SELECT * FROM admin_activity WHERE activity_news_id = $news_id AND activity_user_id=$user_id AND activity_news_type='waiver'  AND activity_action='delete'");
            if($query->num_rows() == 0){
                return $this->db->query("INSERT INTO admin_activity (activity_news_id, activity_user_id, activity_news_type, activity_action) 
                                        VALUES ($news_id, $user_id, 'waiver', 'delete') ");
            }
