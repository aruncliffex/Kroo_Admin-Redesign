...
class Game extends Controller {
    ...
    public function game_add(){
        ...
        $response['insert_id'] = $this->game_model->table_insert('game', $data);
        $response['status'] = true; 
        if($response['insert_id']>0 && $this->input->post('select_game_category')=='1' ){
            //$this->push_notification_game($response['insert_id']);
            for($i=1;$i<=3;$i++){
                $hint_data = array('hint' => $this->input->post('hint'.$i), 'game_id' => $response['insert_id']); 
                $hint_response[$i] = $this->game_model->table_insert('game_levelup_hint', $hint_data);
            }
            $data = array('hint'=>implode(',',$hint_response));
            $this->game_model->table_update('game', $data, 'id', $response['insert_id']);
        }
    ...
    public function game_qustions_update(){
        ...
        $response['insert_id']= $this->game_model->table_update('game', $data, 'id', $id);
        if($response['insert_id']>0 && $this->input->post('select_game_category')=='1'){
            //$this->push_notification_game($response['insert_id']);
            $hint_list = $this->game_model->get_hints($response['insert_id']);
            if(count($hint_list)>0){
                $i=1;
                foreach($hint_list as $hlst){
                    $hint_data = array('hint' => $this->input->post('hint'.$i), 'game_id' => $response['insert_id']); 
                    $hint_response[$i++] = $this->game_model->table_update('game_levelup_hint', $hint_data, 'id', $hlst['id']);
                }
            }else{
                for($i=1;$i<=3;$i++){
                    $hint_data = array('hint' => $this->input->post('hint'.$i), 'game_id' => $response['insert_id']); 
